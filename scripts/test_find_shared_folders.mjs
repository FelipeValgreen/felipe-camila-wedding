...
(async () => {
  ...
  const res = await fetch('https://www.googleapis.com/drive/v3/files?q=mimeType%3D%27application%2Fvnd.google-apps.folder%27&supportsAllDrives=true&includeItemsFromAllDrives=true', {
    headers: { Authorization: `Bearer ${driveToken}` }
  });
  const data = await res.json();
  console.log('SHARED FOLDERS LIST:', JSON.stringify(data, null, 2));
})();
