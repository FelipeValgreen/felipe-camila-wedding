...
(async () => {
  ...
  // Attempt copy with supportsAllDrives
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
  const backupTitle = `BACKUP NATIVO FC - ${timestamp}`;

  const copyRes = await fetch(`https://www.googleapis.com/drive/v3/files/${originalSpreadsheetId}/copy?supportsAllDrives=true`, {
    method: 'POST',
    headers: {
  ...
  console.log('COPY RESULT:', copyRes.status, copyData);
})();
