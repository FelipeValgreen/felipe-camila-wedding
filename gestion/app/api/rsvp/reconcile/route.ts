...
export async function POST(request: Request) {
  try {
    ...
    // Call PostgreSQL transactional RPC function
    const { data: result, error } = await supabase.rpc('reconcile_rsvp_to_guest', {
      p_rsvp_id: rsvp_id,
      p_guest_id: guest_id,
      p_actor: user.email
    });
    ...
    if (error) {
      return NextResponse.json({ ok: false, error: `Transactional reconciliation failed: ${error.message}` }, { status: 500 });
    }
