...
export async function POST(request: Request) {
  try {
    ...
    const insertData: Record<string, any> = {
      ...
      last_name: lastName,
      full_name_normalized: `${firstName} ${lastName}`.trim().toLowerCase(),
      phone_e164: phoneRes.normalized,
      group_name: (body.group_name || 'General').trim(),
      family_side: body.family_side || 'Compartido',
      guest_category: body.guest_category || 'Adulto',
      invitation_status: body.invitation_status || 'not_sent',
      attendance_status: body.attendance_status || 'pending',
      dietary_type: body.dietary_type || 'Ninguna',
      dietary_detail: (body.dietary_detail || '').trim() || null,
      reconfirmation_status: body.reconfirmation_status || 'pending',
      guest_status: body.guest_status || 'active',
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!id || typeof id !== 'string') {
      return NextResponse.json({ ok: false, error: 'ID de invitado inválido o faltante.' }, { status: 400 });
    }

    // Fetch existing guest state
    ...
    // Whitelist payload fields strictly
    ...
    // Validate phone if present in updates
    if ('phone_e164' in updates) {
      const phoneRes = validateAndNormalizePhone(updates.phone_e164);
      if (!phoneRes.valid) {
        return NextResponse.json({ ok: false, error: phoneRes.error }, { status: 400 });
      }
      updates.phone_e164 = phoneRes.normalized;
    }

    // Recalculate full_name_normalized if name changed
    const firstName = updates.first_name !== undefined ? updates.first_name.trim() : beforeGuest.first_name;
    const lastName = updates.last_name !== undefined ? (updates.last_name || '').trim() : (beforeGuest.last_name || '');
    updates.first_name = firstName;
    updates.last_name = lastName;
    updates.full_name_normalized = `${firstName} ${lastName}`.trim().toLowerCase();
    updates.last_dashboard_update_at = new Date().toISOString();
...
export async function DELETE(request: Request) {
  try {
    ...
    if (auth.profile.role !== 'owner') {
      return NextResponse.json({ ok: false, error: 'ONLY_OWNER_CAN_DELETE_GUESTS' }, { status: 403 });
    }

    const { searchParams } = new URL(request.url);
    const id = searchParams.get('id');

    if (!id) return NextResponse.json({ ok: false, error: 'ID de invitado faltante.' }, { status: 400 });

    const { data: beforeGuest } = await supabase.from('wedding_guests').select('*').eq('id', id).single();
    if (!beforeGuest) return NextResponse.json({ ok: false, error: 'Invitado no encontrado.' }, { status: 404 });
