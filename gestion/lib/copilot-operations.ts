...
function decimal(value: string) { return Number(String(value).replace(',', '.')); }
...
function canonicalStatus(value: string, options: Record<string, string>, fallback = value) { return options[normalizeOperationalText(value)] || fallback; }
...
export function parseOperationalAction(text: string): OperationalAction | null {
  ...
  // Domain-specific status commands must be parsed before the generic guest pattern.
  m = clean.match(/(?:marca|actualiza|cambia)\s+(?:la\s+)?(?:canci[oó]n|momento musical)\s+[“\"]?(.+?)[”\"]?\s+como\s+(confirmad[oa]|pendiente|descartad[oa])$/i);
  if (m) { const status = canonicalStatus(m[2], { confirmado: 'Confirmado', confirmada: 'Confirmado', pendiente: 'Pendiente', descartado: 'Descartado', descartada: 'Descartado' }); return makeAction('music.update', `Actualizar música “${m[1].trim()}”`, `Nuevo estado: ${status}`, { musicName: m[1].trim(), updates: { status } }); }
  m = clean.match(/(?:marca|actualiza|cambia)\s+(?:el\s+)?(?:bloque|hito)\s+[“\"]?(.+?)[”\"]?\s+como\s+(confirmado|pendiente|en curso|bloqueado)$/i);
  if (m) { const status = canonicalStatus(m[2], { confirmado: 'Confirmado', pendiente: 'Pendiente', 'en curso': 'En curso', bloqueado: 'Bloqueado' }); return makeAction('timeline.update', `Actualizar bloque “${m[1].trim()}”`, `Nuevo estado: ${status}`, { timelineName: m[1].trim(), updates: { status } }); }
  m = clean.match(/(?:marca|actualiza|cambia)\s+(?:la\s+)?tarea\s+[“\"]?(.+?)[”\"]?\s+como\s+(pendiente|en curso|completada)$/i);
  if (m) { const status = canonicalStatus(m[2], { pendiente: 'Pendiente', 'en curso': 'En curso', completada: 'Completada' }); return makeAction('task.update', `Actualizar tarea “${m[1].trim()}”`, `Nuevo estado: ${status}`, { taskName: m[1].trim(), updates: { status } }); }
  m = clean.match(/(?:marca|actualiza)\s+(?:el\s+)?(?:ítem|item|gasto)\s+[“\"]?(.+?)[”\"]?\s+como\s+(pendiente|confirmado|contratado|pagado|descartado)$/i);
  if (m) { const status = m[2].charAt(0).toUpperCase() + m[2].slice(1).toLowerCase(); return makeAction('budget.update', `Actualizar presupuesto “${m[1].trim()}”`, `Nuevo estado: ${status}`, { budgetName: m[1].trim(), updates: { status } }); }
  m = clean.match(/(?:marca|actualiza|cambia)\s+(?:al\s+)?proveedor\s+[“\"]?(.+?)[”\"]?\s+como\s+(evaluando|contratado|descartado|confirmado)$/i);
  if (m) { const status = m[2].charAt(0).toUpperCase() + m[2].slice(1).toLowerCase(); return makeAction('vendor.update', `Actualizar proveedor “${m[1].trim()}”`, `Nuevo estado: ${status}`, { vendorName: m[1].trim(), updates: { status } }); }

  m = clean.match(/(?:marca|actualiza|cambia)\s+a\s+(.+?)\s+como\s+(confirmad[oa]|asiste|no asiste|pendiente)$/i);
  if (m) { const status = attendance(m[2]); return makeAction('guest.update', `Actualizar a ${m[1].trim()}`, status === 'attending' ? 'Marcar como asistente confirmado.' : status === 'not_attending' ? 'Marcar como no asistente.' : 'Dejar asistencia pendiente.', { guestName: m[1].trim(), updates: { attendance_status: status } }); }
  ...
  if (m) { const amount = m[2] ? money(m[2]) : null; return makeAction('budget.create', `Agregar presupuesto “${m[1].trim()}”`, amount === null ? 'Monto por completar.' : `Monto proyectado: $${Math.round(amount).toLocaleString('es-CL')}`, { item: m[1].trim(), category: 'General', status: 'Pendiente', currency: 'CLP', projectedQuantity: 1, projectedGross: amount, advance: 0 }); }
  m = clean.match(/(?:cambia|actualiza|ajusta)\s+(?:el\s+)?(?:monto|presupuesto)\s+(?:de\s+)?[“\"]?(.+?)[”\"]?\s+(?:a|en)\s+\$?\s*([\d.]+(?:,\d+)?)$/i);
  if (m) { const amount = money(m[2]); return makeAction('budget.update', `Actualizar presupuesto “${m[1].trim()}”`, `Nuevo monto: $${Math.round(amount).toLocaleString('es-CL')}`, { budgetName: m[1].trim(), updates: { projectedGross: amount } }); }
  ...
  if (m) { const amount = money(m[1]); return makeAction('payment.create', `Registrar pago para “${m[2].trim()}”`, `Monto: $${Math.round(amount).toLocaleString('es-CL')}`, { budgetName: m[2].trim(), amount, currency: 'CLP', status: 'Pagado', payment_method: 'Transferencia' }); }
