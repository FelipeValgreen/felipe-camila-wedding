    ...
    items: [
      {
        ...
        description: 'Estado general de la boda',
      },
      {
    ...
    items: [
      {
        ...
        icon: Users,
        description: 'Personas, RSVP y restricciones',
      },
      {
        label: 'Mesas',
        href: '/dashboard/tables',
        icon: Grid,
        description: 'Capacidad y distribución',
      },
    ],
  },
  {
    ...
    items: [
      {
        label: 'Presupuesto y proveedores',
        href: '/dashboard/finance',
        icon: DollarSign,
        description: 'Gastos, pagos y contratos',
      },
      {
        label: 'Actividad',
  ...
  async function handleSyncNow() {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        throw new Error(data.message || data.error || 'No fue posible sincronizar.');
      }
  ...
  const sidebarContent = (
    <>
      <div className="workspace-sidebar-scroll">
        <div className="workspace-brand">
          <div className="workspace-brand-lockup">
            <span className="workspace-mark">F&amp;C</span>
            <div>
              <strong>Centro de Gestión</strong>
              <span>Felipe &amp; Camila</span>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="workspace-event-card">
          <div className="workspace-event-kicker">
            <span>23 octubre 2026</span>
            <span>{daysUntilWedding === 0 ? 'Hoy' : `${daysUntilWedding} días`}</span>
          </div>
          <strong>El Umbral Vivo</strong>
          <p>Una sola vista para decisiones, personas, espacio y operación.</p>
          <div className="workspace-event-progress" aria-hidden="true">
            <span />
          </div>
        </div>
        ...
        <div className="workspace-roadmap">
          <div className="workspace-roadmap-heading">
            <span>En evolución</span>
            <small>Próximos módulos</small>
          </div>
          <div className="workspace-roadmap-list" aria-label="Próximos módulos no disponibles todavía">
            {roadmapItems.map((item) => {
              const Icon = item.icon;
              return (
                <div className="workspace-roadmap-item" key={item.label}>
      ...
      <div className="workspace-sidebar-footer">
        <div className="workspace-assistant-status" aria-label="Asistente de planificación en diseño">
          <span className="workspace-assistant-icon"><Sparkles size={16} /></span>
          <div>
            <strong>Asistente de planificación</strong>
            <small>Identidad y funciones en diseño</small>
          </div>
        </div>
  ...
  return (
    <div className="workspace-shell">
      <header className="workspace-mobile-topbar">
        <button type="button" onClick={() => setMobileOpen(true)} aria-label="Abrir navegación">
          <Menu size={21} />
        </button>
        <div>
          <span>F&amp;C</span>
          <small>{pageContext.title}</small>
        </div>
        <span className="workspace-mobile-days">{daysUntilWedding}d</span>
      </header>
      ...
      <main className="workspace-main">
        <header className="workspace-contextbar">
          ...
          <div className="workspace-context-meta">
            ...
            <span>{daysUntilWedding === 0 ? 'Hoy es el día' : `Faltan ${daysUntilWedding} días`}</span>
          </div>
        </header>
