...
function getDaysUntilWedding() {
  ...
  return Math.max(0, Math.ceil((wedding.getTime() - Date.now()) / 86_400_000));
}
...
export default function DashboardLayout({ children }: LayoutProps) {
  ...
  const pageContext = useMemo(() => {
    ...
    const exact = routeTitles[pathname]; if (exact) return exact;
  ...
  async function handleSyncNow(){setSyncing(true);try{const response=await fetch('/api/sync/process',{method:'POST'});const data=await response.json();if(!response.ok||!data.ok)throw new Error(data.message||data.error||'No fue posible sincronizar.');window.alert(`Sincronización completada. Registros procesados: ${data.processed||0}.`);}catch(error:any){console.error('Error syncing:',error);window.alert(error?.message||'No fue posible sincronizar con Google Sheets.');}finally{setSyncing(false);}}

  const sidebarContent=<>
    <div className="workspace-sidebar-scroll">
      <div className="workspace-brand"><div className="workspace-brand-lockup"><span className="workspace-mark">F&amp;C</span><div><strong>Centro de Gestión</strong><span>Felipe &amp; Camila</span></div></div><button type="button" className="workspace-sidebar-close" onClick={()=>setMobileOpen(false)} aria-label="Cerrar menú"><X size={19}/></button></div>
      <div className="workspace-event-card"><div className="workspace-event-kicker"><span>23 octubre 2026</span><span>{daysUntilWedding===0?'Hoy':`${daysUntilWedding} días`}</span></div><strong>El Umbral Vivo</strong><p>Una sola vista para decisiones, personas, espacio y operación.</p><div className="workspace-event-progress" aria-hidden="true"><span/></div></div>
      <nav className="workspace-nav" aria-label="Navegación del Centro de Gestión">{navigationGroups.map(group=><div className="workspace-nav-group" key={group.label}><span className="workspace-nav-label">{group.label}</span><div className="workspace-nav-list">{group.items.map(item=>{const Icon=item.icon;const isActive=pathname===item.href||(item.href!=='/dashboard'&&pathname?.startsWith(item.href));return<Link key={item.href} href={item.href} className={`workspace-nav-item ${isActive?'is-active':''}`}><span className="workspace-nav-icon"><Icon size={18} strokeWidth={1.8}/></span><span className="workspace-nav-copy"><strong>{item.label}</strong><small>{item.description}</small></span></Link>;})}</div></div>)}</nav>
    </div>
    <div className="workspace-sidebar-footer">
      ...
      <div className="workspace-footer-actions"><button type="button" onClick={handleSyncNow} disabled={syncing} className="workspace-sync-button"><RefreshCw size={15} className={syncing?'animate-spin':''}/><span>{syncing?'Sincronizando…':'Sincronizar datos'}</span></button><a href="https://felipeycami.cl" target="_blank" rel="noreferrer" className="workspace-invite-link"><ExternalLink size={14}/><span>Ver invitación</span></a></div>
      <div className="workspace-profile"><div className="workspace-avatar">{userEmail?.slice(0,1).toUpperCase()||'F'}</div><div className="workspace-profile-copy"><strong>{userEmail}</strong><span>{userRole||'administrador'}</span></div><button type="button" onClick={handleSignOut} aria-label="Cerrar sesión"><LogOut size={16}/></button></div>
    </div>
  </>;
