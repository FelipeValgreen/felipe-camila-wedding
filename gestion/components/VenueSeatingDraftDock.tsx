...
const KEY='fc-preview-seating-proposal-v2';
...
export default function VenueSeatingDraftDock(){
 ...
 return <>{overlay&&canvas?createPortal(overlay,canvas):null}<button type="button" className={styles.fab} onClick={()=>setOpen(v=>!v)}>{open?<X size={17}/>:<><Armchair size={16}/><span>Seating</span>{draft&&<b>{stats.people}</b>}</>}</button>{open&&<aside className={styles.panel}>
  <header><div><span>Mesas → Salón</span><strong>Borrador de distribución</strong><small>{draft?`${label(draft.scenario)} · guardado ${new Date(draft.savedAt).toLocaleString('es-CL')}`:'Todavía no hay un escenario guardado'}</small></div><button onClick={()=>setOpen(false)}><X size={16}/></button></header>
  {!draft?<div className={styles.empty}><Armchair size={22}/><strong>Guarda primero una propuesta en Mesas → IA de mesas.</strong><span>Luego aparecerán aquí la composición social, las mesas adicionales necesarias y el detalle de personas.</span><a href="/dashboard/tables">Abrir Mesas</a></div>:<>
   ...
   <div className={styles.toolbar}><button onClick={()=>setShowGhosts(v=>!v)}>{showGhosts?<EyeOff size={12}/>:<Eye size={12}/>} {showGhosts?'Ocultar mesas propuestas':'Mostrar mesas propuestas'}</button><button onClick={()=>setDraft(readDraft())}><RefreshCw size={12}/>Recargar</button></div>
   ...
   <footer><Users size={14}/><span>El salón representa ubicación física; Mesas mantiene la lógica social. Ambas capas quedan conectadas por este borrador hasta que exista staging para aplicar la propuesta transaccionalmente.</span></footer>
  </>}
 </aside>}</>;
}
