...
const STORAGE_KEY = 'fc-venue-arboleda-layout-v3';
...
const PALETTE: Array<{ kind: VenueKind; label: string }> = [
  { kind: 'stage', label: 'Escenario / DJ' },
  { kind: 'dance', label: 'Pista' },
  { kind: 'bar', label: 'Bar' },
  { kind: 'entrance', label: 'Acceso' },
  { kind: 'cocktail', label: 'Cocktail' },
  { kind: 'lounge', label: 'Living / lounge' },
  { kind: 'buffet', label: 'Buffet / estación' },
  { kind: 'green', label: 'Árbol / planta' },
];
...
export default function VenueEditController() {
  ...
  useEffect(() => {
    try {
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed) && parsed.length) setElements(parsed);
      }
    } catch { /* local draft is optional */ }
  }, []);

  useEffect(() => {
    try { window.localStorage.setItem(STORAGE_KEY, JSON.stringify(elements)); } catch { /* ignore */ }
  }, [elements]);
  ...
  useEffect(() => {
    ...
    const locate = () => {
      if (stopped) return;
      const found = document.querySelector<HTMLElement>('.venue-v2__canvas');
      if (found) { setCanvas(found); return; }
  ...
  useEffect(() => {
    ...
    const previous = legacy.map((item) => item.style.display);
    legacy.forEach((item) => { item.style.display = 'none'; });
  ...
  function onPointerDown(event: React.PointerEvent<HTMLButtonElement>, item: VenueElement) {
  ...
  function onPointerMove(event: React.PointerEvent<HTMLButtonElement>, item: VenueElement) {
    if (!canvas || !drag.current || drag.current.id !== item.id || item.locked) return;
    ...
    update(item.id, { x: Number(x.toFixed(2)), y: Number(y.toFixed(2)) });
  }
  function onPointerUp(event: React.PointerEvent<HTMLButtonElement>, item: VenueElement) {
    if (!drag.current || drag.current.id !== item.id) return;
    drag.current = null;
    if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId);
  }

  function add(kind: VenueKind) {
    checkpoint();
    const next = defaultFor(kind, elements.length + 1);
  ...
  function duplicate() {
    if (!selected) return;
    checkpoint();
    const copy = { ...selected, id: `${selected.kind}-${Date.now()}`, label: `${selected.label} copia`, x: clamp(selected.x + 4, 5, 95), y: clamp(selected.y + 4, 5, 95), locked: false };
    setElements((current) => [...current, copy]); setSelectedId(copy.id);
  }
  function remove() {
    if (!selected) return;
    checkpoint();
  ...
  const overlay = <>
    ...
    <div className={styles.objects}>{elements.map((item) => <button
      ...
      onPointerDown={(event) => onPointerDown(event, item)} onPointerMove={(event) => onPointerMove(event, item)} onPointerUp={(event) => onPointerUp(event, item)} onPointerCancel={(event) => onPointerUp(event, item)}
      onClick={() => setSelectedId(item.id)} title={item.locked ? `${item.label} bloqueado` : `Mover ${item.label}`}
    ><span style={{ transform: `rotate(${-item.rotation}deg)` }}>{item.label}</span>{item.locked && <Lock size={10}/>}</button>)}</div>
    <div className={styles.canvasTools}>
      <button type="button" onClick={() => setShowReference((value) => !value)}>{showReference ? <EyeOff size={13}/> : <Map size={13}/>} {showReference ? 'Ocultar plano oficial' : 'Plano oficial Arboleda'}</button>
      <button type="button" onClick={() => setPanelOpen((value) => !value)}><Layers3 size={13}/> Elementos</button>
    </div>
  </>;

  return <>{createPortal(overlay, canvas)}
    {panelOpen && <aside className={styles.panel}>
      <header><div><span>Editor de capas</span><strong>Elementos del salón</strong><small>Borrador autosguardado en este navegador</small></div><button type="button" onClick={() => setPanelOpen(false)}><X size={16}/></button></header>
      ...
      <footer><strong>Referencia real</strong><span>El plano oficial del recinto puede superponerse como guía. Los elementos interiores siguen siendo una propuesta editable hasta reconciliar el plano/presentación exactos del montaje.</span></footer>
    </aside>}
  </>;
}
