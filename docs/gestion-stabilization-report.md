...
- **Storage Bucket `wedding-photos`**: Se revocó el listado público broad (`storage.objects` listing) limitando la gestión a los administradores autenticados (`Admin Manage wedding-photos storage`). Se comprobó la carga activa de las fotografías históricas del matrimonio civil.
...
- **Exportación XLSX Local**: Generado archivo `backups/FC_Centro_Comandos_Backup_Official.xlsx` (230 KB) en el repositorio.

---

## 6. Procedimiento de Rollback Reproducible

En caso de requerir reversión inmediata:
1. **Script SQL de Reversión**: Ejecutar `supabase/migrations/rollback/20260723020000_rollback_security_hardening.sql`.
...
3. **Respaldo Google Sheets**: Restaurar desde el archivo local `backups/FC_Centro_Comandos_Backup_Official.xlsx` o desde las pestañas nativas `BK_MAESTRA_...` en el spreadsheet `1bbzTemOTWdE-QSy2L1u_B6Oc4s3j4o3oSOLBAdTl7F0`.
