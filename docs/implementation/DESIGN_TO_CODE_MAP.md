...
| **Interactive RSVP Modal** | `/` -> `components/RSVPModal.tsx` | Supabase `rsvp_guests` (Server Action) | `isSubmitting` (bool), `isAttending` (bool) | Center modal on desktop, full-screen slide-up on mobile. | Focus traps inside modal; keyboard escape route; form labels linked to inputs. | Medium. Need to handle slow-network latency. |
...
| **Background Music widget** | Global layout -> `components/AudioPlayer.tsx` | `assets/song.mp3` | `isPlaying` (bool), `volume` (float) | Compact floating player near FAB. | Play/pause easily triggerable via keyboard; screen reader descriptions. | Low. Autoplay blocking policy handles fallback. |
...
Global Context (App State)
  ├── Guest Identity (Token -> guestName, passes)
