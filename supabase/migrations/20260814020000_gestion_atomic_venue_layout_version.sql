...
begin
  ...
  if p_space_width_m <= 0 or p_space_width_m > 500 then
    raise exception 'SPACE_WIDTH_INVALID';
  end if;
  if p_space_height_m <= 0 or p_space_height_m > 500 then
    raise exception 'SPACE_HEIGHT_INVALID';
  end if;
  if p_grid_step_m <= 0 or p_grid_step_m > 25 then
    raise exception 'GRID_STEP_INVALID';
  end if;

  -- Serialize version creation so two editors cannot both archive/create an
  -- active layout concurrently. The unique partial index remains the final DB guard.
...
grant execute on function public.create_venue_layout_version(text,text,jsonb,text,text,text,numeric,numeric,numeric) to service_role;
